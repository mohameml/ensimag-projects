...
public abstract class Tree {
    ...
    String prettyPrintNode() {
              // j'ai ajouté d'ici
        if(this.getClass().getSimpleName().equals("DeclField"))
        {
            DeclField field = (DeclField) this;
            if(field.getVisibility() == Visibility.PROTECTED)
            {
                return "[Visibility=PROTECTED] " + this.getClass().getSimpleName() ;
            }else{
                return "[Visibility=PUBLIC] " + this.getClass().getSimpleName() ;
            }
        }
