...
def extract_build_solution(zipped_folder_path, destination_folder_name):
    ...
    try:
        ...
        if not Path(new_folder).exists():
            ...
            sys.exit(1)
        os.chdir(new_folder)
        print(f'Building solution in folder {unzipped_folder} (platform Any CPU)')
        subprocess.run("dotnet build /nowarn:msb3246,msb3270 /p:Platform=\"Any CPU\" -v q", check=True)
...
def create_grpc_client_path(grpc_folder_path):
    ...
    print(f'Building client in folder {grpc_folder_path} (platform Any CPU)')
    subprocess.run("dotnet build /nowarn:msb3246,msb3270 /p:Platform=\"Any CPU\" -v q", check=True)
    print(f"Done building client in folder {grpc_folder_path}")
    grpc_client_path=grpc_folder_path.joinpath('GrpcEvaluation/bin/Debug/net8.0')
